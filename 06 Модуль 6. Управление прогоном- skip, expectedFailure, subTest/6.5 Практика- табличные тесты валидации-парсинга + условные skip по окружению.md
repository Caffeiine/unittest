...
```mermaid
flowchart TD
    A[unittest loader нашёл test_метод] --> B[TextTestRunner запускает TestCase.run]
    B --> C{Условия окружения ок?}
    C -- нет --> D[skip / skipIf / skipUnless / skipTest]
    C -- да --> E[цикл по кейсам]
    E --> F[with self.subTest(params)]
    F --> G[Act: вызов парсера/валидатора]
    G --> H{Assert}
    H -- ok --> I[addSubTest(..., outcome=None)]
    H -- fail/error --> J[addSubTest(..., outcome=exc_info)]
    I --> E
    J --> E
    E --> K[итоговый отчёт: failures/errors/skipped]
```
